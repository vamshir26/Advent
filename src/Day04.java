	...
	public static void main(String[] args) {
		...
		for (String[] each : input) {
			...
			for (int i = 0; i < each.length - 1; i++) {
				each[i].chars().filter(x -> x != '-')
						.forEach(x -> freq.put((char) x, freq.getOrDefault((char) x, 0) + 1));
			}

			// get top 5 frequencies, sorted by character order and then
			// frequency
			// and turn into string
			String top5 = freq.entrySet().stream().sorted(Map.Entry.comparingByKey())
					.sorted(Collections.reverseOrder(Map.Entry.comparingByValue())).limit(5).map(Map.Entry::getKey)
					.map(x -> Character.toString(x)).collect(Collectors.joining(""));
